...
if ( isset( $_POST['name'], $_POST['domain'], $_POST['tld'], $_POST['password'] ) ) {
    $name     = trim( preg_replace( '/[^a-z^A-Z^0-9^-]/', "", strtolower( $_POST['name'] ) ) );
    $domain   = trim( preg_replace( '/[^a-z^A-Z^0-9^-]/', "", strtolower( $_POST['domain'] ) ) );
    $tld      = trim( preg_replace( '/[^a-z^A-Z^0-9^-]/', "", strtolower( $_POST['tld'] ) ) );
    $password = trim( preg_replace( '/[^a-z^A-Z^0-9^-]/', "", ( $_POST['password'] ) ) );
    } else {
